...
///
/// ## Persistence
/// When enabled by Settings, only text-like entries are saved in a small
/// per-user file. Image bytes, file paths, and pasteboard items marked
/// concealed, transient, or auto-generated never reach that file.
@MainActor
final class ClipboardMonitor: ObservableObject {
    /// Newest-first. Capped at `historyLimit` — see `record(_:)`.
    @Published private(set) var entries: [ClipboardEntry] = []

    /// Hard cap on the in-memory history. Persistence uses the same item cap,
    /// with stricter content and size rules below.
    static let historyLimit = 50

    /// Persisted text is intentionally smaller than the in-memory copy-back
    /// cap. This keeps the saved file useful for normal snippets without
    /// turning it into a general document store.
    static let persistentStringCap = 32_000

    /// Keep saved clipboard data for at most 30 days. A copied secret should
    /// not remain on disk forever just because the user forgot about it.
    static let persistenceRetention: TimeInterval = 30 * 24 * 60 * 60

    /// A saved history stays small enough to inspect, replace, and remove
    /// quickly. Images and files are excluded before this limit is applied.
    static let persistentByteBudget = 1_000_000
    ...
    /// item carrying any of them is skipped entirely, never captured, never
    ...
    init(pasteboard: NSPasteboard = .general, persistenceURL: URL? = nil) {
    ...
    /// Enables or disables the saved history. The setting is separate from
    /// collection so a user can keep the widget running without leaving a
    /// clipboard file on disk. Disabling it deletes the saved file at once.
    func setPersistenceEnabled(_ enabled: Bool) {
        if persistenceEnabled == enabled {
            if !enabled { removePersistedHistory() }
            return
        }
    ...
    private func restorePersistedEntries() {
        guard let data = try? Data(contentsOf: persistenceURL) else { return }

        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        guard let history = try? decoder.decode(PersistedHistory.self, from: data),
              history.version == 1 else {
            removePersistedHistory()
            return
        }

        let cutoff = Date().addingTimeInterval(-Self.persistenceRetention)
        let restored = history.entries
            .filter { $0.capturedAt >= cutoff }
            .compactMap(Self.makeClipboardEntry)
        ...
        if entries.isEmpty {
            entries = Array(restored.prefix(Self.historyLimit))
        } else {
            entries = Array((entries + restored)
                .sorted { $0.capturedAt > $1.capturedAt }
    ...
    private static func makeClipboardEntry(_ persisted: PersistedEntry) -> ClipboardEntry? {
        guard let kind = ClipboardEntry.Kind(rawValue: persisted.kind),
              [.text, .url, .color].contains(kind),
              !persisted.text.isEmpty,
              persisted.text.count <= persistentStringCap else { return nil }
        ...
        return ClipboardEntry(
            ...
            kind: kind,
            preview: kind == .color
                ? persisted.text.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
                : singleLinePreview(persisted.text),
            fullString: persisted.text,
            filePaths: nil,
            colorComponents: kind == .color
                ? parseHexColor(persisted.text.trimmingCharacters(in: .whitespacesAndNewlines))
                : nil)
    }
    ...
    private func persistHistory() {
        ...
        let cutoff = Date().addingTimeInterval(-Self.persistenceRetention)
        var records = entries.compactMap { entry -> PersistedEntry? in
            guard entry.capturedAt >= cutoff,
                  [.text, .url, .color].contains(entry.kind),
                  let fullString = entry.fullString,
                  !fullString.isEmpty else { return nil }

            let text: String
            if fullString.count > Self.persistentStringCap {
                text = String(fullString.prefix(Self.persistentStringCap)) + "…"
            } else {
                text = fullString
            }

            // A truncated URL would no longer copy back as a useful link.
            if entry.kind == .url && text != fullString { return nil }
            return PersistedEntry(capturedAt: entry.capturedAt,
        ...
        func encode(_ records: [PersistedEntry]) -> Data? {
            try? encoder.encode(PersistedHistory(version: 1, entries: records))
        }

        while !records.isEmpty,
              let data = encode(records), data.count > Self.persistentByteBudget {
            records.removeLast()
        }

        guard let data = encode(records), !records.isEmpty else {
            removePersistedHistory()
            return
        }
        ...
        do {
            ...
            clipboardLog.error("Clipboard history persistence failed: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private var isConcealedOrTransient: Bool {
        ...
        return items.contains { item in
            item.types.contains(Self.concealedType)
                || item.types.contains(Self.transientType)
                || item.types.contains(Self.autoGeneratedType)
        }
    }
    ...
    /// Image payloads never enter the persisted history, so a run of
    /// full-resolution retina screenshots stays in memory only. This is the
    /// budget the old "no image payload at all" decision was avoiding;
    /// setting it explicitly is better than refusing the feature.
    ...
    /// Empties history entirely and removes the saved copy. It does not touch
    /// the live pasteboard itself.
    func clear() {
        entries.removeAll()
        removePersistedHistory()
    }
