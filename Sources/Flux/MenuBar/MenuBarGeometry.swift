...
/// The safe status-item span used while Flux hides icons on macOS 27.
///
/// macOS 27 drops a status item when its length reaches a display-specific
/// cliff. It no longer clamps a huge divider and slides the icons away. Several
/// bounded items are needed instead, so every item stays below the cliff while
/// their combined width reaches the usable menu-bar area.
